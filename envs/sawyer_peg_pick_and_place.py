...
class SawyerPegPickAndPlaceV2(SawyerXYZEnv):
  ...
  def __init__(self, reward_type='dense', full_state_goal = False, reset_at_goal=False):

    hand_low = (-0.5, 0.20, 0.05)
    hand_high = (0.5, 1, 0.5)
    obj_low = (-0.4, 0.2, 0.01478)
    obj_high = (0.4, 1.0, 0.4)
    goal_low = (-0.6, 0.2, 0.01478)
    goal_high = (0.6, 1.0, 0.4)
    ...
    if full_state_goal:
      goal_states = np.array([[0.4, 0.8, 0.2, 1.0, -0.3, 0.4, 0.2]])
    else:
      goal_states = np.array([[-0.3, 0.4, 0.2]])
