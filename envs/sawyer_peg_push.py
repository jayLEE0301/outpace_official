...
class SawyerPegPushV2(SawyerXYZEnv):
  max_path_length = int(1e8)
  TARGET_RADIUS = 0.05

  def __init__(self, reward_type='dense', reset_at_goal=False, full_state_goal = False, close_gripper = False):

    hand_low = (-0.5, 0.20, 0.05)
    hand_high = (0.5, 1, 0.5)
    obj_low = (-0.4, 0.2, 0.01478)
    obj_high = (0.4, 1.0, 0.01479)
    goal_low = (-0.6, 0.2, 0.01478)
    goal_high = (0.6, 1.0, 0.01479)
    ...
    self.init_config = {
        'obj_init_pos': np.array([0.4, 0.8, 0.02]),
        # 'hand_init_pos': np.array([0, 0.6, 0.2]),
        'hand_init_pos': np.array([0.4, 0.8, 0.2]),
    }

    self.initial_states = initial_states
    if full_state_goal:
      goal_states = np.array([[0.4, 0.8, 0.2, 1.0, -0.3, 0.4, 0.02]])
    else:
      goal_states = np.array([[-0.3, 0.4, 0.02]])
    ...
    if full_state_goal:
      self._target_pos = self.goal[4:].copy()
    else:
    ...
    self.goal_space = Box(
      np.array(goal_low), #+ np.array([.03, .0, .0]),
      np.array(goal_high) #+ np.array([.03, .0, .0])
    )
  ...
  def model_name(self):
    return os.path.join(
      os.path.dirname(os.path.realpath(__file__)), "metaworld_assets/sawyer_xyz", 'sawyer_peg_push.xml')
  ...
  def get_next_goal(self):
    if not self._reset_at_goal:
      return self.goal_states[0].copy()
  ...
  def reset_goal(self, goal=None, add_noise = False):
    ...
    if self.full_state_goal:
      ...
      # self._target_gripper_distance = goal[3]
      self._target_pos = goal.copy() #goal[4:]
      # self._end_effector_goal = goal[:3]
  ...
  def evaluate_state(self, obs, action):
    ...
    reward, tcp_to_obj, tcp_open, obj_to_target, grasp_reward, in_place_reward, collision_box_front, ip_orig, is_success = self.compute_reward(obs, action)
  ...
  def reset_model(self, goal = None):    
    if not self._reset_at_goal:
      ...
      self._set_obj_xyz(self.obj_init_pos + np.concatenate([np.random.uniform(-0.05, 0.05, size=2), np.zeros([1])], axis=-1))
      self.peg_head_pos_init = self._get_site_pos('pegHead')
    else:
      raise NotImplementedError
      self._reset_hand()
  ...
  def compute_reward(self, obs, action=None):
    ...
    obj_head = obs[4:7] 
    tcp_opened = obs[3]
    if self.full_state_goal: # actually same both if & else
      target = obs[11:14]
    else:
      target = obs[-3:]
    ...
    is_success = self.is_successful(obs=obs)  

    return [reward, tcp_to_obj, tcp_opened, obj_to_target, object_grasped, in_place, collision_boxes, ip_orig, is_success]
  ...
  def is_successful(self, obs=None):
    ...
    if self.full_state_goal: # actually same both if & else
      ...
      return np.linalg.norm(obs[..., 4:7] - obs[..., -3:], axis =-1) <= self.TARGET_RADIUS
